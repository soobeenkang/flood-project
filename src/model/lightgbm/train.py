...
# Optuna용 샘플
# ───────────────────────────────────────────────
def stratified_sample(df, n_total=500_000, neg_pos_ratio=10, random_state=42):
    pos = df[df[TARGET_COL] == 1]
    neg = df[df[TARGET_COL] == 0]

    if len(pos) >= n_total:
        return pos.sample(n=n_total, random_state=random_state).reset_index(drop=True)

    n_neg = min(len(neg), len(pos) * neg_pos_ratio, n_total - len(pos))
...
def main():
    ...
    if not os.path.exists(param_path):
        print("\nRunning Optuna...")
        sample_df = stratified_sample(train_df, n_total=500_000, neg_pos_ratio=10)
